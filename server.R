...
server <- function(input, output) {
  ...
  outputOptions(output, "scenario_type", suspendWhenHidden = FALSE)
  ...
  observeEvent(input$cmip, {
    if (input$cmip == "cmip6"){
      ...
      # Year
      output$cmip_year <- renderUI({
        radioButtons("year_type", "Year",
                            inline = TRUE,
                            c("2021-2040" = "2021.2040",
                              "2041-2060" = "2041.2060",
                              "2061-2080" = "2061.2080",
                              "2081-2100" = "2081.2100"),
                            selected = "2081.2100")
      ...
      output$cmip_sce <- renderUI({
      # SSP
        radioButtons("scenario_type", "Shared Socioeconomic Pathway (SSP)",
                            inline = TRUE,
                            c("SSP 1" = "ssp126",
                              "SSP 2" = "ssp245",
                              "SSP 3" = "ssp370",
      ...
      output$selection_gcms <- renderUI({
        ...
        scenarios <- list.files(paste0("clim_data/worldclimCMIP6"), pattern = input$res_sel) %>%
          dplyr::as_tibble() %>%
        ...
        #        scenario_ = scenario_ %>% str_replace_all("rcp", ""))
    ...
    if (input$cmip == "cmip5"){
      ...
      # RCP
      output$cmip_sce <- renderUI({
        radioButtons("scenario_type", "Representative Concentration Pathway (RCP)",
                            inline = TRUE,
                            c("RCP 2.6" = "rcp26",
                              "RCP 4.5" = "rcp45",
                              "RCP 6.0" = "rcp60",
      ...
      output$selection_gcms <- renderUI({
        glue::glue("# Getting list of available GCMs") %>% message
        scenarios <- list.files(paste0("clim_data/worldclimCMIP5"), pattern = input$res_sel) %>%
          dplyr::as_tibble() %>%
          magrittr::set_names("GCM") %>%
          tidyr::separate(col = GCM, into = c("wc_", "resolution_", "bio_", "gcm_", "scenario_", "year_"), sep = "_") %>% dplyr::select(-wc_, -bio_) %>%
          slice(-1)
  ...
  observe({
    ...
    output$selected_scenario <- renderText(
      glue::glue("Current selection: Year {input$year_type}, scenario {input$scenario_type}, resolution: {input$res_sel}") %>% print()
    )
  ...
  output$compare_type_bio_bio <- renderUI({
    conditionalPanel(condition = "input.compare_type == 'bio_bio'",
                     selectInput(inputId = "bio_bio_x", 
                                 label = "X axis",
                                 choices = c("(bio 1) Annual Mean Temperature" = "bio1",
                                             "(bio 2) Mean Diurnal Range (Mean of monthly (max temp - min temp))" = "bio2",
                                             "(bio 3) Isothermality (BIO2/BIO7) (* 100)" = "bio3",
                                             ...
                                             "(bio 7) Temperature Annual Range (BIO5-BIO6)" = "bio7",
                                             ...
                                             "(bio 15) Precipitation Seasonality (Coefficient of Variation)" = "bio15",
  ...
  reactive({
    ...
    rvs$def_available_gcms <- scenarios %>% 
      ...
      # filter(year_ == "2070") %>% 
      filter(year_ == "2081.2100") %>% 
      # filter(scenario_ == "45") %>% 
      filter(scenario_ == "ssp245") %>% 
      pull(gcm_)
  ...
  observeEvent(input$go, {
    # For debugging
    # input$bio_bio_x <- "bio1"
    # input$bio_bio_y <- "bio12"
    ...
    # bio_vars_x2 <- case_when(bio_vars_x == "(bio 1) Annual Mean Temperature" ~ "bio1",
    #                          bio_vars_x == "(bio 2) Mean Diurnal Range (Mean of monthly (max temp - min temp))" ~ "bio2",
    #                          bio_vars_x == "(bio 3) Isothermality (BIO2/BIO7) (* 100)" ~ "bio3",
    #                          bio_vars_x == "(bio 4) Temperature Seasonality (standard deviation *100)" ~ "bio4",
    #                          bio_vars_x == "(bio 5) Max Temperature of Warmest Month" ~ "bio5",
    #                          bio_vars_x == "(bio 6) Min Temperature of Coldest Month" ~ "bio6",
    #                          bio_vars_x == "(bio 7) Temperature Annual Range (BIO5-BIO6)" ~ "bio7",
    #                          bio_vars_x == "(bio 8) Mean Temperature of Wettest Quarter" ~ "bio8",
    #                          bio_vars_x == "(bio 9) Mean Temperature of Driest Quarter" ~ "bio9",
    #                          bio_vars_x == "(bio 10) Mean Temperature of Warmest Quarter" ~ "bio10",
    #                          bio_vars_x == "(bio 11) Mean Temperature of Coldest Quarter" ~ "bio11",
    #                          bio_vars_x == "(bio 12) Annual Precipitation" ~ "bio12",
    #                          bio_vars_x == "(bio 13) Precipitation of Wettest Month" ~ "bio13",
    #                          bio_vars_x == "(bio 14) Precipitation of Driest Month" ~ "bio14",
    #                          bio_vars_x == "(bio 15) Precipitation Seasonality (Coefficient of Variation)" ~ "bio15",
    #                          bio_vars_x == "(bio 16) Precipitation of Wettest Quarter" ~ "bio16",
    #                          bio_vars_x == "(bio 17) Precipitation of Driest Quarter" ~ "bio17",
    #                          bio_vars_x == "(bio 18) Precipitation of Warmest Quarter" ~ "bio18",
    #                          bio_vars_x == "(bio 19) Precipitation of Coldest Quarter" ~ "bio19")
    # bio_vars_y2 <- case_when(bio_vars_y == "(bio 1) Annual Mean Temperature" ~ "bio1",
    #                          bio_vars_y == "(bio 2) Mean Diurnal Range (Mean of monthly (max temp - min temp))" ~ "bio2",
    #                          bio_vars_y == "(bio 3) Isothermality (BIO2/BIO7) (* 100)" ~ "bio3",
    #                          bio_vars_y == "(bio 4) Temperature Seasonality (standard deviation *100)" ~ "bio4",
    #                          bio_vars_y == "(bio 5) Max Temperature of Warmest Month" ~ "bio5",
    #                          bio_vars_y == "(bio 6) Min Temperature of Coldest Month" ~ "bio6",
    #                          bio_vars_y == "(bio 7) Temperature Annual Range (BIO5-BIO6)" ~ "bio7",
    #                          bio_vars_y == "(bio 8) Mean Temperature of Wettest Quarter" ~ "bio8",
    #                          bio_vars_y == "(bio 9) Mean Temperature of Driest Quarter" ~ "bio9",
    #                          bio_vars_y == "(bio 10) Mean Temperature of Warmest Quarter" ~ "bio10",
    #                          bio_vars_y == "(bio 11) Mean Temperature of Coldest Quarter" ~ "bio11",
    #                          bio_vars_y == "(bio 12) Annual Precipitation" ~ "bio12",
    #                          bio_vars_y == "(bio 13) Precipitation of Wettest Month" ~ "bio13",
    #                          bio_vars_y == "(bio 14) Precipitation of Driest Month" ~ "bio14",
    #                          bio_vars_y == "(bio 15) Precipitation Seasonality (Coefficient of Variation)" ~ "bio15",
    #                          bio_vars_y == "(bio 16) Precipitation of Wettest Quarter" ~ "bio16",
    #                          bio_vars_y == "(bio 17) Precipitation of Driest Quarter" ~ "bio17",
    #                          bio_vars_y == "(bio 18) Precipitation of Warmest Quarter" ~ "bio18",
    #                          bio_vars_y == "(bio 19) Precipitation of Coldest Quarter" ~ "bio19")
    bio_vars_x2 <- bio_vars_x
    bio_vars_y2 <- bio_vars_y
  ...
  observeEvent(input$go, {
    withProgress(message = 'Loading climatic data',
                 ...
                 value = 0, {
                   ...
                   print(input$res_sel)
                   print(input$sel_gcms)
                   print(input$scenario_type)
                   print(input$year_type)
                   print(input$cmip)
                   print(rvs$bio_vars_x2)
                   print(rvs$bio_vars_y2)
                   print(rvs$polySelXY)
                   ...
                   if(!is.null(input$sel_gcms)){
                     ...
                     if(input$cmip == "cmip6"){
                       clim_vars_files <- paste0("wc2.1_",
                                                 ...
                                                 "_bioc_",
                                                 input$sel_gcms %>% gsub("-", "\\.", .), 
                                                 "_", input$scenario_type, "_",
                                                 input$year_type %>% gsub("-", "\\.", .)) %>%
                     ...
                     if(input$cmip == "cmip6"){
                       ...
                       rvs$clim_vars_complete <- clim_vars_files %>%
                         magrittr::set_names(rvs$def_available_gcms)
                     }
                   ...
                   glue::glue("#>>   Checking if cropping is needed") %>% message
                   # req(rvs$polySelXY)
                   ...
                   ### Divide by 10 some temperature layers to use common units  -- Not necessary in WC2.0
                   
                   if(input$cmip == "cmip5"){
                     glue::glue("#>>  Dividing temperature layers in CMIP5 by 10 to get units in ºC") %>% message
                     for (i in 1:length(rvs$clim_vars)){
                       for (j in names(rvs$clim_vars[[i]])[names(rvs$clim_vars[[i]]) %in% c("bio1", "bio2", "bio4", "bio5", "bio6", "bio7", "bio8", "bio9", "bio10", "bio11")]){
                   ...
                   if(input$cmip == "cmip5"){
                     for (j in names(rvs$clim_baseline)[names(rvs$clim_baseline) %in% c("bio1", "bio2", "bio4", "bio5", "bio6", "bio7", "bio8", "bio9", "bio10", "bio11")]){
  ...
  observeEvent({#input$type_scaled
    ...
    withProgress(message = 'Comparing GCM values',
                 ...
                 value = 0, {
                   ...
                   if(input$compare_type == "bio_bio"){
                     rvs$xaxis_lab <- case_when(rvs$bio_vars_x2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_x_full, " (ºC)"),
                                                rvs$bio_vars_x2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_x_full, " (mm)"),
                                                rvs$bio_vars_x2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_x2, ""))
                     rvs$yaxis_lab <- case_when(rvs$bio_vars_y2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_y_full, " (ºC)"),
                                                rvs$bio_vars_y2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_y_full, " (mm)"),
                                                rvs$bio_vars_y2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_y2, ""))
                   }
                   if(input$compare_type == "bio_several"){
                   ...
                   if(input$compare_type == "bio_bio"){
                     ...
                     rvs$xcol_unsc_lab <- case_when(rvs$bio_vars_x2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_x2, " (ºC)"),
                                                    rvs$bio_vars_x2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_x2, " (mm)"),
                                                    rvs$bio_vars_x2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_x2, ""))
                     rvs$ycol_unsc_lab <- case_when(rvs$bio_vars_y2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_y2, " (ºC)"),
                                                    rvs$bio_vars_y2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_y2, " (mm)"),
                                                    rvs$bio_vars_y2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_y2, ""))
                     
                     rvs$xcol_delta_lab <- case_when(rvs$bio_vars_x2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_x2, " (ºC) - delta"),
                                                     rvs$bio_vars_x2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_x2, " (mm) - delta"),
                                                     rvs$bio_vars_x2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_x2, " - delta"))
                     rvs$ycol_delta_lab <- case_when(rvs$bio_vars_y2 %in% paste0("bio", c(1:2, 5:11)) ~ paste0(rvs$bio_vars_y2, " (ºC) - delta"),
                                                     rvs$bio_vars_y2 %in% paste0("bio", c(12:19)) ~ paste0(rvs$bio_vars_y2, " (mm) - delta"),
                                                     rvs$bio_vars_y2 %in% paste0("bio", c(3:4)) ~ paste0(rvs$bio_vars_y2, " - delta"))
                   }
                   if(input$compare_type == "bio_several"){
      ...
      output$gcm_patterns <- renderUI({
        ...
        for (b in names(rvs$clim_vars[[1]])){
          ...
          scenario_data <- rvs$clim_vars %>%
            ...
            purrr::reduce(c) %>%
            magrittr::set_names(names(rvs$clim_vars))
          ...
          long_name <- case_when(b == "bio1" ~ "Annual Mean Temperature (bio 1, °C) ",
                                 ...
                                 b == "bio3" ~ "Isothermality (bio 3, bio2/bio7 * 100)",
                                 b == "bio4" ~ "Temperature Seasonality (bio 4, standard deviation *100)",
                                 b == "bio5" ~  "Max Temperature of Warmest Month (bio 5, °C) ",
                                 b == "bio6" ~  "Min Temperature of Coldest Month (bio 6, °C)",
                                 b == "bio7" ~  "Temperature Annual Range (bio 7, °C)",
                                 b == "bio8" ~  "Mean Temperature of Wettest Quarter (bio 8, °C)",
                                 b == "bio9" ~  "Mean Temperature of Driest Quarter (bio 9, °C)",
                                 b == "bio10" ~ "Mean Temperature of Warmest Quarter (bio 10, °C)",
                                 b == "bio11" ~ "Mean Temperature of Coldest Quarter (bio 11, °C)",
                                 b == "bio12" ~ "Annual Precipitation (bio 12, mm)",
                                 b == "bio13" ~ "Precipitation of Wettest Month (bio 13, mm)",
                                 b == "bio14" ~ "Precipitation of Driest Month (bio 14, mm)",
                                 b == "bio15" ~ "Precipitation Seasonality (bio 15, coefficient of variation)",
                                 b == "bio16" ~ "Precipitation of Wettest Quarter (bio 16, mm)",
                                 b == "bio17" ~ "Precipitation of Driest Quarter (bio 17, mm)",
                                 b == "bio18" ~ "Precipitation of Warmest Quarter (bio 18, mm)",
          ...
          scenario_plot <- ggplot() + 
            ...
            labs(title = paste0(long_name, " - Unmodified GCMs")) +
    ...
    output$delta_patterns <- renderUI({
      ...
      for (b in names(rvs$clim_delta[[1]])){
        ...
        scenario_data <- rvs$clim_delta %>%
          purrr::map(~ terra::subset(.x, b)) %>%
          purrr::reduce(c) %>%
          magrittr::set_names(names(rvs$clim_delta))
        ...
        # Plot
          ...
          tidyterra::geom_spatraster(data = scenario_data) + facet_wrap(~lyr) + tidyterra::scale_fill_wiki_c(name = "") + theme_bw()+
          scale_x_continuous(n.breaks=3, guide = guide_axis(check.overlap = TRUE)) +
          labs(title = paste0(long_name, " - Variation from current climate")) +
          theme(axis.text.x = element_blank(), axis.text.y = element_blank()) 
    ...
    output$deltaperc_patterns <- renderUI({
      ...
      for (b in names(rvs$clim_deltaperc[[1]])){
        ...
        long_name <- case_when(b == "bio1" ~ "Annual Mean Temperature (bio 1) ",
                               ...
                               b == "bio3" ~ "Isothermality (bio 3, bio2/bio7 * 100)",
                               b == "bio4" ~ "Temperature Seasonality (bio 4, standard deviation *100)",
                               b == "bio5" ~  "Max Temperature of Warmest Month (bio 5) ",
                               b == "bio6" ~  "Min Temperature of Coldest Month (bio 6)",
                               b == "bio7" ~  "Temperature Annual Range (bio 7)",
                               b == "bio8" ~  "Mean Temperature of Wettest Quarter (bio 8)",
                               b == "bio9" ~  "Mean Temperature of Driest Quarter (bio 9)",
                               b == "bio10" ~ "Mean Temperature of Warmest Quarter (bio 10)",
                               b == "bio11" ~ "Mean Temperature of Coldest Quarter (bio 11)",
                               b == "bio12" ~ "Annual Precipitation (bio 12)",
                               b == "bio13" ~ "Precipitation of Wettest Month (bio 13)",
                               b == "bio14" ~ "Precipitation of Driest Month (bio 14)",
                               b == "bio15" ~ "Precipitation Seasonality (bio 15, coefficient of variation)",
                               b == "bio16" ~ "Precipitation of Wettest Quarter (bio 16)",
                               b == "bio17" ~ "Precipitation of Driest Quarter (bio 17)",
  ...
  observe({
    ...
    print_scenario <- input$scenario_type %>% str_replace("rcp", "") %>% str_replace("ssp", "")
    output$selected_scenario_2_tab <- renderText(
      glue::glue("Year {input$year_type}, scenario{print_scenario}, resolution: {input$res_sel}") %>% print()
    )
  ...
  output$download_prec_temp_unscaled <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_fig_DifPreA_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".png")
    },
  ...
  output$download_prec_temp_deltas <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_fig_DifPreB_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".png")
    },
    content = function(file) {
      ggsave(file,
  ...
  output$download_prec_temp_scaled <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_fig_DifFutA_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".png")
    },
    content = function(file) {
      ggsave(file,
  ...
  output$download_prec_temp_realunscaled <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_fig_DifFutB_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".png")
    },
    content = function(file) {
      ggsave(file,
  ...
  output$download_comparison_table_no_scaled <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_table_DifPre_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".csv")
    },
    content = function(file) {
  ...
  output$download_comparison_table <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_table_DifFut_scaled_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".csv")
    },
    content = function(file) {
  ...
  output$download_comparison_table_realunscaled <- downloadHandler(
    filename = function() {
      paste0("GCM_compareR_table_DifFut_unscaled_", input$year_type, "_", input$scenario_type, "_", format(Sys.time(), "%Y-%m-%d"), ".csv")
    },
    content = function(file) {
